...
- **The bound on answers that mail alone starts (^9bxas0w).** Mail causes the next submission with no caller call, and that stays the normal case. But a model that starts one more background run in each answer (for example, it asks a background status tool again after each result) gets one more answer for each run, with no end, and holds the model the whole time. ^3qx0mpt saw it: one test ran more than 250 delivery submissions after its assertions. The pump counts the answers in a row that mail alone started (`RoutedSessionActor.mailOnlyAnswersInARow`). An answer that delivered a caller message, also one that joined a continuation, sets the count to zero. When the count reaches `SessionConfiguration.mailOnlyAnswerLimit` and the next batch holds mail only, the pump puts that mail back into the outbox, held (`SessionOutbox.putBack(holding:)`), and starts no answer. A held terminal starts no submission by itself, so the pump ends. The next caller message carries the held mail into its submission, and the count starts again. The mail is never lost. The session reports each hold with `SessionEvent.mailDeliveryPaused(MailDeliveryPause)` on `streamSessionEvents()` (no answer runs, so no answer stream carries it) and with a log line of category `MailDelivery`. The default limit is `SessionConfiguration.defaultMailOnlyAnswerLimit`, 100: a normal chain (a model starts some runs, and each settled run starts one answer) is short, so only a chain with no end reaches it. `0` holds all mail for the next caller message. The sidecar records the limit (a sidecar written before it restores with the default), a restore applies it again, and a fork inherits it. Tests: `MailOnlyAnswerLimitTests`, `MailDeliveryPausePublicSurfaceTests`, `SessionTreeRestorationTests.restoredTreeReappliesRecordedMailOnlyAnswerLimit`.
