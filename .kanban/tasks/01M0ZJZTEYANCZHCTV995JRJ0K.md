...
- actor: claude-code
  id: 01m0zmr6vc50194tmnagcwfq74
  text: |-
    ### review — findings
    - evidence: `review sha HEAD~1..HEAD` on commit `284eb5f`, 7 files. The validator fleet found 0 (7 attempted, 0 failed, 0 skipped), as the card predicted: no rule can tell which fact a deleted sentence carried. The directed check of the kept-fact list against `git show HEAD~1:<path>` found 5. Four are lost facts: `Recording/MergedTranscript.swift:53` (the `- Throws:` on public `merged(under:)` lost "otherwise if a transcript file cannot be read", and one unreadable file aborts the whole merge); `Recording/Sinks.swift:86` (the `- Throws:` on `init(owningDirectory:now:)` lost the untyped file-system error class, so an exhaustive `catch RecordingRootLockError` now reads as complete); `Recording/Sinks.swift:75` (the owning initializer no longer states it creates the root and its lock marker on disk, against a sibling initializer that still promises "writing nothing to disk until an event lands"); `Hosting/ToolInvocationRecord.swift:29` (the reason for `Codable` is gone, and public `TurnOutcome.toolInvocations` is the direct surfacing it named). The fifth is `Session/ToolOutputCapping.swift:99`: the kept-fact list's last four line references are off by +3 and two published counts are stale, both from the reversal that restored `- Throws:` on `TokenCappingTool.call`; the file is 67/130 and the set is 465/965, not 64/127 and 462/962.
    - checks that passed: comment-only diff is 0 lines; 0 public, `open` or `package` symbols without a doc comment; all four `- Parameter` keys name the internal parameter; all three moved facts landed where claimed; the reversed `- Throws:` is in the tree; all 80 kept facts are present in the files.
...
## Review Findings (2026-08-26 13:17)
...
- Comment-only: `git diff -U0 HEAD~1..HEAD -- Sources` with the `///` lines filtered out is 0 lines. No code line changed.
- No public, `open` or `package` symbol lost its doc comment. Scanned every such declaration across the seven files: 0 undocumented. Enum cases and protocol requirements keep theirs as well. The symbols that lost a doc entirely are all internal or private — `MergedTranscriptError.errorDescription`, `TokenCappingTool.wrapped`, `TokenCappingTool.limit`, `productionOpenHandle`, `InMemoryRecorder.seq`, `NoneRecorder.append` — and each deleted line restated a protocol requirement or an empty body.
- Every surviving `- Parameter <name>:` names the internal parameter, not the argument label. All four: `mountedTools` and `property` in `DiscoveryPriming.swift` (no separate label), `eventSeq` in `Sinks.swift` against label `afterSeq`, `routerDirectory` in `MergedTranscript.swift` against label `under`.
- All three MOVED facts landed where claimed. The `nil maxTokens` rule is on the protocol header of `LanguageModelSessionBackend.swift` and covers all four generating methods that take `maxTokens: Int?`. The pending-envelope exemption is at `TokenCappingTool/call(arguments:)` and keeps the control-plane reason, the `completionToken`, and `PendingRunEnvelope/isRendered(text:)`. The schema-version error is named on the public `merged(under:)`.
- The reversed cut is in the tree. `ToolOutputCapping.swift` reads "- Throws: Whatever `wrapped` throws, unmodified. This decorator wraps the output, never the error." The second sentence carries more than the line it replaced.
- Kept-fact list, content: all 80 entries are present in the files. Only the four `ToolOutputCapping.swift` line references above are wrong.

### The three accepted judgement calls
