...
- actor: claude-code
  id: 01kzrqgnv2d0f0yanycf0206cp
  text: |-
    Worked all 14 open findings from the two dated sections. Decisions on the two findings with design content:

    **SessionOutbox.swift:421 / :435 (drain pair asymmetry)** — made the pair symmetric at `internal`, not both public. `drainForDispatch()` and `finishDispatch()` are both explicitly `internal` now, and `drainForDispatch()`'s doc states the contract that binds a drain to a turn: every drain happens inside `dispatchNextPrompt()`'s `beginTurn()`/`endTurn()` bracket and is released by exactly one `finishDispatch()` on every exit of that bracket. This closes the public-API reconstruction of the drained-but-not-started window structurally — an external caller can no longer drain at all. `Drained` followed its only producer to `internal`. Rationale against the other direction (making `finishDispatch()` public): a public release would let an external caller empty the slot out from under a running dispatch, which corrupts `queueDepth()` the opposite way. Tests were unaffected — `SessionOutboxTests` uses `@testable import`. Swept the same cause across the file: `journalWithoutStaging`, `requeue`, and `attach(journal:)` (each a deliberately internal route on the public actor) are now explicitly `internal` too.

    **RoutedSessionActorGeneration.swift:315 (text events bypass the session feed)** — took the correct-the-documentation route, not the routing route, deliberately. Routing `.textDelta`/`.textReset` into `streamSessionEvents()` would contradict that feed's own load-bearing buffering rationale: it buffers every subscription `.unbounded` so a slow consumer drops nothing, and that is affordable exactly because a turn contributes a handful of lifecycle events there rather than per-token text (stated at `RoutedSessionActorGeneration.swift` on `streamSessionEvents()`). Per-token deltas into an unbounded buffer with a slow subscriber is unbounded memory. So the exclusion is now stated as contract in all four places the false claim lived or the mechanism sits: `RoutedSession.swift` `streamSessionEvents()` doc (new "Excluded, deliberately" paragraph with the reason and what a text-needing consumer uses instead), `SessionEvent.swift` type doc, `turnEventSink(_:)` doc plus `runTurn`'s `onEvent` parameter doc, and `sessionEvents(for:)` where the bypass physically happens.

    **RoutedSessionActorTurnExecution.swift:769 (false turn-cancelled window)** — documented, not closed. Closing analysis: the slot lives on `SessionOutbox` and `currentTurnId` on `RoutedSessionActor`, so the release is inherently a cross-actor suspension; reordering it after `endTurn()` is worse — the slot would then name a finished prompt while an unrelated next turn runs, making `cancelPrompt` cancel an innocent turn. The current order confines the ambiguity to the one suspension. Documented at the call site and on `cancelPrompt(id:)`: `turnCancelled` is a *request* report with exactly `TurnCancellationResult.requested`'s semantics, never an outcome; the completed-turn window is named explicitly; the summary line no longer claims "at any point before its turn generates".

    **RoutedSession.swift:814** — the parked-dispatch bullet now states both cases: the parked call dispatches the next queued prompt instead, or returns `nil` when the withdrawn prompt was the only one queued.

    **SessionEvent.swift:27** — source-compatibility acknowledgement added to `SessionEvent`'s own doc: public enum without library evolution, adding a case breaks external exhaustive switches, `turnStarted(_:)` was added this way, `default` is the absorb-future-cases option.

    Mechanical fixes: `internal func runCancellableModelCall(`, `internal func finishDispatch()`, `internal init(_ value: UInt64)` on `TurnID` (with the intent stated); CompactionDemo named constants `demoContextTokens`, `progressPollMilliseconds`, `compactionTriggerFill`/`compactionTargetFill`; `// periphery:ignore` (reason on its own comment line, no trailing text, per the dead-code-swift staging contract) above the `(docReply, docFill, docCompactions)` binding, which IS read two lines down — annotate-never-delete per project convention.

    Verified after the changes: `swift build --build-tests` clean; ungated `swift test` green — 811 tests / 77 suites (one pre-existing known issue in BoundedWait), 27 / 11, 24 / 5, zero failures; `diagnostics check working` 0 errors 0 warnings. No id-space mixing introduced: no new id fields at all, only access levels and documentation plus example-file constants. NOT run: gated suites, swift format, check-doc-links. Nothing pushed, nothing committed.
  timestamp: 2026-08-11T15:37:03.586911+00:00
- actor: claude-code
  id: 01kzrqgvjqvp7g240af6dw8scm
  text: |
    ### implement — changed
