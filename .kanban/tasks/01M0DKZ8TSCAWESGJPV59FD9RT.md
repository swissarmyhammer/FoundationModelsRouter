...
- [ ] The flash-tier summary quality hazard is documented on `performAutoCompaction` and on the compaction plan, or a guard makes the hazard unreachable
- [ ] The demo trio (`SmolLM-135M` as flash) is checked in every place that pairs it with a `budget:` opt-in
#compaction