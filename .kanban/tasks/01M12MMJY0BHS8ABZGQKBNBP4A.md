...
- Instrument `RoutedSessionActor.runTurn` (Sources/FoundationModelsRouter/Session/RoutedSessionActorTurnExecution.swift:103). Every surface uses this path: `respond(to:maxTokens:)`, `streamResponse(to:maxTokens:)`, `streamEvents(to:maxTokens:)`, and `dispatchNextPrompt()`. Confirm the delivery-turn path (`runDispatchedTurn`, line 534) also flows through it; if it does not, instrument that path too.
...
- After a good turn, set the measured token counts from the turn's usage snapshot (see `usageState` and Tests/FoundationModelsRouterTests/TokenUsageMeteringTests.swift) as attributes, for example `tokens.in` and `tokens.out`.
...
- Do not put prompt text or response text in an attribute.
...
- [ ] An application without a tracing backend sees no change in behavior.
