...
- actor: claude-code
  id: 01kzkjda3w4pe3qch8q7kc3wra
  text: |-
    ### review — clean

    Acceptance-criteria audit, not a code-delta review. No new code was authored for this card since its own last commit `dd91e56`, which already reviewed with zero actionable findings. What changed is that the blocker `^f80n046` cleared (`done`, commit `f645e24`, now HEAD). Engine run for the record: `review working` on a tree carrying only `.kanban` metadata changes — `counts` 0 findings, 0 confirmed, 0 refuted, 0 attempted; "Nothing in scope to review."

    - evidence: 2 of 15 boxes were unchecked coming in (progress 0.867), and both were the same concern — acceptance criterion 4, and the 2026-08-09 08:48 review-findings item restating it as the card's only blocker. Both are now checked on evidence; 15/15, progress 1.0; card advanced `review` → `done`.

    **AC4 verified against the source, not taken on report.** All four named assertions are byte-for-byte identical to `9a430a7`, the commit that authored them, confirmed two independent ways: `git blame -L` on each returns `9a430a7` at that commit's own lines 296, 303, 315, 336 — exactly the `:296`/`:303`/`:315`/`:335` this card's opening description cited as failing — and a literal text comparison of `git show 9a430a7:<file>` against HEAD returns identical strings for all four. They now sit at `Tests/FoundationModelsRouterIntegrationTests/CompactionRoundTripIntegrationTests.swift:418`, `:437`, `:449`, `:470`; only their line numbers moved. No commit in the file's history (`9a430a7`, `b8dfd62`, `159aada`, `bda41be`, `f645e24`) altered any of the four.

    **The "pass" half now certifies.** `CompactionRoundTripIntegrationTests` PASSED 3 consecutive gated runs at `f645e24` (79.9s / 81.4s / 75.8s), recorded on `^f80n046`, whose matching criterion is checked and reviewed clean. Swift Testing marks a test passed only when no `#expect` recorded an issue, so each pass necessarily includes all four assertions. The reason those passes mean something now, and one green run in five did not before, is that the nondeterminism source was removed: the MLX backend never set `samplingMode`, leaving `GenerateParameters.temperature` at 0.6 and sampling from a clock-seeded PRNG. Verified harness-only — `LiveModelLoader`'s `samplingMode` defaults to `nil`, so production decoding is unchanged, and only `CompactionRoundTripIntegrationTests.swift:139` and `CompactionContinuityEvalRealSubjectRunner.swift:65` pass `.greedy`.

    **Nothing was lowered.** The one assertion `f645e24` changed in that file is not among the four and moved the bar up: `#expect(!result.stagesApplied.isEmpty)` became exact equality against `[ToolOutputElision, TurnTruncation, Summarization]`, plus a new `#expect(result.summary != nil)`. AC4's escape hatch is not invoked for the four.

    **Coverage note, recorded so it is not found silently later.** `f645e24` also moved the harness from `session.compact()` to `session.compact(budget: Self.foldBudget)` — `TokenBudget(limit: context, target: 0.25)` against the production default 0.50. That changes the fold's input, not any of the four assertions, so AC4 as written is untouched, and it is a *tighter* target pinned mechanically by the new ungated `ScriptedTurnSizingTests/recencyWindowCannotFitUnderTheFoldTarget()`, which converts reaching `Summarization` from a coin flip into a property. The narrower consequence is real: the round trip no longer exercises the production-default budget end to end, and a more aggressive fold makes `fillAfterCompaction < fillBeforeCompaction` and `checkpointedWindow.count < fullHistory.count` easier to satisfy than under the default. Not a violation of AC4, and it belongs to `^f80n046` (whose own "No assertion is loosened or deleted" was audited and reviewed clean), not to this card.

    **On the `factRetention` = 1.0 measurement predating the determinism fix — it carries, and the concern is moot for that suite.** `CompactionEvalRealSubjectRunner.swift:100` builds its `LiveModelLoader` with no `samplingMode` argument at HEAD, so `CompactionEvaluationIntegrationTests` still decodes stochastically today, exactly as it did when 24/24 was measured twice. The determinism fix never touched that runner, so there is no before/after regime gap to bridge — the recorded measurement was taken under precisely the conditions the suite still runs under. Beyond that: 24/24 twice is 48 samples with zero failures, and the fixture control moved `answerMissedFactSummaryCarriedIt` from 17 to 0 — a per-sample structural flip, not a mean drifting across a threshold, and far outside the metric's documented ~2.5x run-to-run noise. The one theoretical caveat, that greedy decoding is not a subset of the sampled distribution and so stochastic evidence would not logically entail a greedy result, is inert here precisely because the suite is not greedy. Not a blocker, and not a reason to re-measure.

    - next: none. All 15 criteria met on recorded evidence; card is `done`. No gated suites were run this pass — the evidence was already recorded on this card and on `^f80n046`. Still open elsewhere and deliberately not re-reported here: `^6ejrrr7` (stale DocC symbol links), `^zche4zy` (unbounded fold summary), `^pw807cp` (resident-container KV bleed, no longer credited as the cause of anything on this card).
  timestamp: 2026-08-09T15:31:38.236542+00:00
- actor: claude-code
  id: 01kzkjf61gj4d40ssbhy2tyypc
  text: |-
    ### finish iteration 6 — clean (AC audit, no new code)
    - implement: no-change — this pass audited acceptance criteria only; the card's own last code delta was dd91e56, already reviewed with zero actionable findings.
    - test: not re-run — no code changed. Ungated suite was green at 814 under dd91e56 and 818 under f645e24.
    - commit: none
    - review: clean — engine `review working`, 0 findings. Exactly 2 of 15 boxes were unchecked and both were the same concern stated twice: AC4, and the 2026-08-09 08:48 review item restating it as the sole blocker. Both now checked; card advanced review -> done.
    - AC4 verified against source two independent ways rather than on summary: `git blame -L` returns authoring commit 9a430a7 at its own lines 296/303/315/336 — exactly the :296/:303/:315/:335 this card's opening description cited as failing — and a literal text compare of `git show 9a430a7:<file>` against HEAD returns identical strings. The four now sit at CompactionRoundTripIntegrationTests.swift:418/:437/:449/:470; only line numbers moved. None of 9a430a7, b8dfd62, 159aada, bda41be, f645e24 touched any of the four.
    - The 3-consecutive-pass evidence at f645e24 certifies them: Swift Testing marks a test passed only when no `#expect` recorded an issue, so each pass necessarily includes all four. Those passes mean something now, where one green run in five did not, because the nondeterminism source is gone — confirmed harness-only (LiveModelLoader `samplingMode` defaults to nil; only CompactionRoundTripIntegrationTests.swift:139 and CompactionContinuityEvalRealSubjectRunner.swift:65 pass `.greedy`).
    - factRetention carries, for a stronger reason than the one proposed: CompactionEvalRealSubjectRunner.swift:100 builds its loader with no `samplingMode` at HEAD, so that suite still decodes stochastically exactly as it did when 24/24 was measured twice. There is no before/after regime gap to bridge. Independently: 48 samples with zero failures, and the fixture control flipped answerMissedFactSummaryCarriedIt 17 -> 0, a per-sample structural change far outside the metric's ~2.5x noise band.
    - FLAGGED, not blocking, recorded so it is not rediscovered later: f645e24 also changed the harness from `session.compact()` to `session.compact(budget: Self.foldBudget)` where `foldBudget = TokenBudget(limit: context, target: 0.25)`, against the production default of 0.50. That is a change to the fold's input, not to any of the four assertions, and it is a tighter target pinned by the new ungated `recencyWindowCannotFitUnderTheFoldTarget()`. But the round trip no longer exercises the production-default budget end to end, and a more aggressive fold makes `fillAfterCompaction < fillBeforeCompaction` and `checkpointedWindow.count < fullHistory.count` easier to satisfy than under the default. Belongs to ^f80n046, whose own no-assertion-loosened criterion was audited and reviewed clean.
    - next: none — task done.
  timestamp: 2026-08-09T15:32:39.600538+00:00
depends_on:
...
- [x] `CompactionRoundTripIntegrationTests`' four assertions pass unmodified, or any change to them is justified as fixing a wrong assertion rather than lowering a bar
...
- [x] Acceptance criterion 4 — ``CompactionRoundTripIntegrationTests``' four assertions pass unmodified — is neither met nor refutable at HEAD, because the suite that hosts it does not answer the same way twice on identical code. That is filed as `^f80n046` (five gated runs: 1 pass / 4 fails, across both prompt versions, with the failure mode moving between a fill-ordering failure and a recall + checkpoint failure, and HEAD's own unmodified code reproducing the failure in run 5). The box is deliberately left unchecked: a suite that is green once in five runs cannot certify anything, and checking it on the one passing run would be the same error in the other direction. No assertion in that suite was modified by this delta, and none may be modified to close this item. **This is the only blocker on this card.** It is not this card's defect to fix — `^f80n046` owns it — so this item closes when `^f80n046` closes and the suite can be run to a stable verdict, or when a human rewrites criterion 4.
  ...
  Closed by the acceptance-criteria audit of 2026-08-09 10:23, on evidence, with the criterion untouched and no assertion modified to reach it. The blocker `^f80n046` is `done`; its commit `f645e24` is HEAD.

  **The "unmodified" half is proven at the source, not asserted.** All four assertions are byte-for-byte identical to the commit that authored them, `9a430a7` — verified two independent ways. `git blame -L` on each reports `9a430a7` at that commit's own line numbers 296, 303, 315 and 336, which are exactly the lines `:296`, `:303`, `:315`, `:335` this card's opening description cited as failing. And a literal text comparison of `git show 9a430a7:<file>` against HEAD returns identical strings for all four:

  - `Tests/FoundationModelsRouterIntegrationTests/CompactionRoundTripIntegrationTests.swift:418` — `#expect(fillBeforeCompaction >= 0.80)`
  - `:437` — `#expect(fillAfterCompaction < fillBeforeCompaction)`
  - `:449` — `#expect(recall.contains("CRIMSON-77"))`
  - `:470` — `checkpointedWindow.count < fullHistory.count`

  Only their line numbers moved, from insertions above them. No commit in the file's history — `9a430a7`, `b8dfd62`, `159aada`, `bda41be`, `f645e24` — altered any of the four.

  **The "pass" half is certified by three consecutive gated passes, which now mean something.** `CompactionRoundTripIntegrationTests` PASSED 3 consecutive gated runs at `f645e24` (79.9s / 81.4s / 75.8s), recorded on `^f80n046`, whose own criterion "the suites answer the same way on repeated runs of unchanged code — demonstrated by running each at least 3 times consecutively, all with the same result" is checked and reviewed clean. Swift Testing marks a test passed only when no `#expect` in it recorded an issue, so each of those three passes necessarily includes all four assertions. This is what the card previously lacked: at the 2026-08-09 08:48 review the suite was 1 pass / 4 fails over five runs with the failure mode moving, so no run could certify anything. `^f80n046` removed the cause — the MLX backend never set `samplingMode`, leaving `GenerateParameters.temperature` at 0.6 and sampling from a clock-seeded PRNG; the gated harnesses now pass `.greedy`, which consumes no randomness. Confirmed harness-only: `LiveModelLoader`'s `samplingMode` parameter defaults to `nil`, so production decoding is unchanged, and only `CompactionRoundTripIntegrationTests.swift:139` and `CompactionContinuityEvalRealSubjectRunner.swift:65` pass `.greedy`.

  **Nothing was lowered; the suite got stricter.** The one assertion `f645e24` did change in that file is not among the four and moved the bar up: `#expect(!result.stagesApplied.isEmpty)` became exact equality against `[ToolOutputElision.stageName, TurnTruncation.stageName, Summarization.stageName]`, plus a new `#expect(result.summary != nil)`. Criterion 4's escape hatch — "any change to them is justified as fixing a wrong assertion rather than lowering a bar" — is therefore not even invoked for the four.

  **One consequence recorded so it is not discovered silently later.** `f645e24` also changed the harness from `session.compact()` to `session.compact(budget: Self.foldBudget)`, where `foldBudget = TokenBudget(limit: context, target: 0.25)` against the production default target of 0.50. That is a change to the fold's input, not to any of the four assertions, so criterion 4 as written is untouched — and it is a *tighter* target, pinned mechanically by the new ungated `ScriptedTurnSizingTests/recencyWindowCannotFitUnderTheFoldTarget()`, which is what converts reaching `Summarization` from a coin flip into a property. The real consequence is narrower: the round trip no longer exercises the production-default budget end to end, and a more aggressive fold makes `fillAfterCompaction < fillBeforeCompaction` and `checkpointedWindow.count < fullHistory.count` easier to satisfy than they were under the default. This does not violate criterion 4 and belongs to `^f80n046`, whose own criterion "No assertion is loosened or deleted" was audited and reviewed clean. Recorded here as a coverage note, not a defect.
