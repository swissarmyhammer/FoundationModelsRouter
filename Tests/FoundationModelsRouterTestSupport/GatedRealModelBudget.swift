/// The generation budget of each answer for the gated real-model suites.
///
/// Both gated test targets read this constant. One shared value prevents drift
/// between the many answer sites.
///
...
public enum GatedRealModelBudget {
    /// The largest number of response tokens one gated answer can generate.
    ///
    ...
    /// reasoning, and the answer records an empty response. An empty response
    ...
    /// model's end-of-sequence token, and an answer that stops early costs only
    /// the tokens it generated. A larger ceiling therefore gives space to the
    /// `<think>` block, and it does not make a short answer longer.
    ///
