...
///
/// The same variable ``CompactionSmokeIntegrationTests`` and
/// ``AutoCompactionTriggerIntegrationTests`` read, and deliberately NOT
/// `FM_ROUTER_INTEGRATION_TESTS`. All three suites answer one question — does
/// compaction work at all against a real model — and all three answer it in
/// seconds, so one variable turns on the whole fast tier.
/// `FM_ROUTER_INTEGRATION_TESTS` also turns on the 15-to-30-minute suites of
/// this target, which is why none of the three reads it.
///
/// Kept as its own file-scoped constant rather than shared with either
/// neighbour's, because Swift's top-level `private` is file-scoped, not
/// target-scoped.
private let recordedTranscriptCompactionEnvVar = "FM_ROUTER_COMPACTION_SMOKE"

private var recordedTranscriptCompactionEnabled: Bool {
    ProcessInfo.processInfo.environment[recordedTranscriptCompactionEnvVar] != nil
}
...
@Suite(
    "Gated real-model smoke test: a recorded transcript boots the compaction fold (task ^pfdrppj)",
    .timeLimit(.minutes(recordedTranscriptCompactionTimeLimitMinutes)),
    .enabled(if: recordedTranscriptCompactionEnabled),
    .exclusiveRealModel
)
