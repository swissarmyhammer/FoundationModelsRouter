...
/// every integration run, over whatever recording is checked in. A
/// re-recorded fixture that carries an operator path, a credential shape, or
/// the `recordingRoot` leak goes red here before it ships.
///
    ...
    private static func recordedFiles() throws -> [URL] {
        let fixtureRoot = try #require(
            Bundle.module.resourceURL?.appendingPathComponent(
                RecordedTranscriptCompactionIntegrationTests.recordingResourcePath,
                isDirectory: true),
