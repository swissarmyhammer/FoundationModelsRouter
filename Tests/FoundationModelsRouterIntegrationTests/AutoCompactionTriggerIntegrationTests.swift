...
/// The opt-in environment variable that enables this suite.
///
/// The same variable ``CompactionSmokeIntegrationTests`` reads, and deliberately
/// NOT `FM_ROUTER_INTEGRATION_TESTS`. Both suites answer one question — does
/// compaction work at all against a real model — and both answer it in seconds,
/// so one variable turns on the whole fast tier. `FM_ROUTER_INTEGRATION_TESTS`
/// also turns on the 15-to-30-minute suites of this target, which is why
/// neither suite reads it.
///
/// Kept as its own file-scoped constant rather than shared with the smoke
/// suite's, because Swift's top-level `private` is file-scoped, not
/// target-scoped.
private let autoCompactionTriggerEnvVar = "FM_ROUTER_COMPACTION_SMOKE"

private var autoCompactionTriggerEnabled: Bool {
    ProcessInfo.processInfo.environment[autoCompactionTriggerEnvVar] != nil
}
...
@Suite(
    "Gated real-model smoke test: a synthetic trigger folds a short transcript inside its own turn (task ^d02ryqj)",
    .timeLimit(.minutes(autoCompactionTriggerTimeLimitMinutes)),
    .enabled(if: autoCompactionTriggerEnabled),
    .exclusiveRealModel
)
