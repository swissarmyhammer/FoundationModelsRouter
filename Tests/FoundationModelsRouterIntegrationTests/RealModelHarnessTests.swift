...
/// A ``LanguageModelSessionBackend`` that generates nothing.
///
/// ``RealModelHarnessTests`` never drives a session — it reads the profile
/// ``RealModelHarness`` builds and the `session.json` that profile's recording
/// writes — so this exists only to satisfy
/// ``StubLoadedContainer``'s two factory methods. No test asserts on anything
/// this returns.
private final class StubSessionBackend: LanguageModelSessionBackend {
    ...
    func makeFork() -> any LanguageModelSessionBackend { StubSessionBackend() }
...
/// A ``LoadedLLMContainer`` holding no model at all.
///
/// This is the whole reason ``RealModelHarness/make(model:context:container:cacheDir:recordingsDir:routerId:)``
/// takes the protocol rather than the concrete ``MLXFoundationModelsContainer``:
/// the concrete type wraps a real resident MLX model and cannot be constructed
/// without one, so a suite that supplied it could only run behind the 20-minute
/// gate. The harness uses nothing of the container except to hand it to each
/// generation handle, so a stand-in proves every fact the harness stamps.
///
/// It is the seam the router itself documents for this — see
/// ``LoadedLLMContainer/makeSession(instructions:)``, which states that unit
/// tests inject a stub container.
private struct StubLoadedContainer: LoadedLLMContainer {
    func makeSession(instructions: String?) -> any LanguageModelSessionBackend { StubSessionBackend() }
...
/// `any LoadedLLMContainer` so a stand-in satisfies it. Every fact the two
/// suites depend on is then readable here in milliseconds.
///
