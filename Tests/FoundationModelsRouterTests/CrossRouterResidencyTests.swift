...
struct CrossRouterResidencyTests {
    ...
    func releaseFromOneRouterKeepsTheModelForTheOther() async throws {
        ...
        var fromFirst: LanguageModelProfile? = try await pair.first.resolve(
            profile: Self.sharedTrio, reporting: ResolutionProgress())
        let fromSecond = try await pair.second.resolve(profile: Self.sharedTrio, reporting: ResolutionProgress())

        fromFirst.dropReference()
        await pair.first.pool.settleDroppedResidencies()
    ...
    func lastReleaseEvictsThroughTheLoadingRoutersLoader() async throws {
        ...
        var fromFirst: LanguageModelProfile? = try await pair.first.resolve(
            profile: Self.sharedTrio, reporting: ResolutionProgress())
        var fromSecond: LanguageModelProfile? = try await pair.second.resolve(
            profile: Self.sharedTrio, reporting: ResolutionProgress())
        ...
        await pair.second.pool.settleDroppedResidencies()
