...
/// its append handle at each turn close (a `.response`-kind event), and
/// ``TranscriptTree`` tolerates the crash artifact that policy expects — a
/// torn final line in a `transcript.jsonl` — while failing loudly, with the
/// session id and file path, on corruption anywhere before it.
...
struct RecordingDurabilityTests {
    ...
    /// One recorded session on disk: its id, directory, and transcript file.
    ...
    func tornFinalLineIsDroppedOnLoad(keptBytes: Int) async throws {
        ...
        let lastLineStart = Self.finalLineStart(of: data)
        #expect(keptBytes < data.count - lastLineStart)
        try data.prefix(lastLineStart + keptBytes).write(to: fixture.transcriptURL)
    ...
    func midFileCorruptionThrowsTypedError() async throws {
        ...
        let text = try String(contentsOf: fixture.transcriptURL, encoding: .utf8)
        ...
        try Data((lines.joined(separator: "\n") + "\n").utf8).write(to: fixture.transcriptURL)
