...
/// Task ^cvtfem3: pins the claim a `RoutedSession` answer is supposed to make —
...
///
/// Both tests drive the same scripted multi-call answer against the same
/// mounted tool and assert the same thing about the answer, by content: it
/// carries the marker text of *every* tool output the answer produced. Holding
/// the two surfaces to one assertion is the point — a surface that drops tool
/// output, or a loop that delivers only the first output, fails here while the
/// other surface passes, and the difference is the defect.
...
struct ToolOutputFeedbackTests {
    ...
    /// The steps one answer calls the mounted tool for, in order. Two of them,
    /// so an answer is `call -> output -> call again -> output -> answer` and a
    /// loop that delivers only the first output is caught.
    ...
    /// The answer shape: one round per step, each round asking for a single
    /// call on the mounted tool with that step as its argument.
    private static var script: ScriptedAnswerScript {
    ...
    /// Asserts the one contract both surfaces are held to: the answer really
    /// made a call per step, and the answer carries every tool output's marker.
    ///
    /// - Parameters:
    ///   - answer: The final text of the answer.
    ///   - tool: The mounted tool the answer called.
    private static func expectEveryToolOutputReachedTheAnswer(
        _ answer: String, tool: MarkerEmittingTool
    ) {
        // The answer is genuinely multi-call: one tool call per step, in order.
    ...
    @Test("respond(to:) answers with every mounted tool output the answer produced")
    ...
    @Test("streamEvents(to:) answers with every mounted tool output the answer produced")
