...
/// test needs to read the answer back and to clean up after it.
///
...
/// — so an answer driven through it crosses Router's real session machinery,
/// with the scripted model standing in only for the weights.
struct ScriptedSessionFixture {
    ...
    /// SDK's own transcript back off the session its answer ran through.
    let vendedBackends: VendedBackendLog

    /// The recorder the fixture's router persists every transcript event
    /// into, so a test can assert on exactly what an answer recorded.
    ...
    ///
    /// Read off the vended backend, so it is the same transcript the
    /// submission chokepoint diffed. Only call this once the answer has
    /// returned — the discipline ``LanguageModelSessionBackend/transcriptEntries()``
    /// documents.
    ///
    ...
    static func make(
        playing script: ScriptedAnswerScript,
