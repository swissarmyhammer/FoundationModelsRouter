...
/// Task ^ake8sax: the stall watch measures generation, so it runs only while a
/// pass holds its place in the generation queue (`generation-queue.md`,
/// section 2). A wait for a queue place and a tool body between two passes
/// are not a stall. A session tells its consumer that a pass waits for a
/// queue place with ``SessionEvent/passQueued``, and that the pass took the
/// place with ``SessionEvent/passStarted``.
///
/// Each session is a routed session over one ``LiveBackendContainer``, so each
/// pass goes through the production backend and its per-session
/// ``QueuedLanguageModel``, which reports the pass to the session.
///
...
@Suite("A wait for a queue place is not a stalled generation (task ^ake8sax)")
struct QueuedPassStallWatchTests {
    ...
    /// How long a test holds a session in a queue wait or in a tool body:
    ...
    /// The prompt of the session that holds the queue place.
    private static let holdingPrompt = "a"

    /// The prompt of the session that waits for the queue place.
    ...
    @Test(
        "a streaming request that waits for a queue place longer than the stall interval reports the wait and no stall",
        .timeLimit(.minutes(1)))
    func aWaitForAQueuePlaceIsNotAStall() async throws {
        ...
        let waitReported = await BoundedWait.conditionReached("the report of the wait") {
            await waitingLog.contains(.passQueued)
        }
        ...
        #expect(waitingEvents.filter { $0 == .passQueued }.count == 1)
        #expect(waitingEvents.filter { $0 == .passStarted }.count == 1)
        let queuedAt = try #require(waitingEvents.firstIndex(of: .passQueued))
        let startedAt = try #require(waitingEvents.firstIndex(of: .passStarted))
    ...
    @Test(
        "a pass that holds its queue place and makes no fragment still reports a stall",
        .timeLimit(.minutes(1)))
    ...
    @Test(
        "after a queue wait and a tool body, a stall measures only the held pass, and its time in flight is the whole call",
        .timeLimit(.minutes(1)))
    func aStallAfterAWaitAndAToolBodyMeasuresOnlyTheHeldPass() async throws {
        ...
        // waiting session then holds its place until the last step.
        step.signal()
        step.signal()
