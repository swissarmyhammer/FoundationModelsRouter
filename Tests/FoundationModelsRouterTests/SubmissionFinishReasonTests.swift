...
/// A submission that runs out of output tokens must not look like a
/// submission that finished.
///
...
/// than one generation call in one submission, so the tests of a tool loop
/// prove that the count of the last call decides, and not the sum of all calls.
@Suite("Submission finish reason: a truncated submission is distinguishable from a finished submission")
struct SubmissionFinishReasonTests {
    /// The prefix of each temp directory this suite makes.
    private static let tempDirPrefix = "SubmissionFinishReasonTests"
    ...
    private static func metadataOnlyReason(_ entries: [Transcript.Entry]) -> FinishReason {
        FinishReason(submissionEntries: entries, outputTokens: nil, lastCallOutputTokens: nil, responseTokenCeiling: nil)
    }

    // MARK: - The reading of the entries of one submission
    ...
    func flaggedOutputBelowCeilingIsEndedInsideReasoning() {
        let reason = FinishReason(
            submissionEntries: Self.endedInsideReasoningEntries, outputTokens: Self.outputBelowCeiling,
    ...
    func flaggedOutputAtCeilingIsMaxTokens() {
        let reason = FinishReason(
            submissionEntries: Self.endedInsideReasoningEntries, outputTokens: Self.requestedCeiling,
            lastCallOutputTokens: Self.requestedCeiling, responseTokenCeiling: Self.requestedCeiling)

        #expect(reason == .maxTokens)
    }

    @Test("a last response with no metadata ends the submission as completed")
    ...
    @Test("an incompleteOutput value of false ends the submission as completed")
    ...
    @Test("a flagged response that a later response follows does not end the submission")
    ...
    @Test("a submission with no response entry ends as completed")
    func noResponseIsCompleted() {
        #expect(Self.metadataOnlyReason([Self.reasoning("thinking")]) == .completed)
    }

    // MARK: - The output token count of one submission against its ceiling
    ...
    func outputAtCeilingIsMaxTokens() {
        ...
        let reason = FinishReason(
            submissionEntries: entries, outputTokens: Self.requestedCeiling, lastCallOutputTokens: nil,
    ...
    func outputBelowCeilingIsCompleted() {
        ...
        let reason = FinishReason(
            submissionEntries: entries, outputTokens: Self.outputBelowCeiling, lastCallOutputTokens: nil,
            responseTokenCeiling: Self.requestedCeiling)
    ...
    func unknownOutputIsCompleted() {
        ...
        let reason = FinishReason(
            submissionEntries: entries, outputTokens: nil, lastCallOutputTokens: nil,
            responseTokenCeiling: Self.requestedCeiling)
    ...
    func toolLoopLastCallAtCeilingIsMaxTokens() {
        let reason = FinishReason(
            submissionEntries: Self.toolLoopEntries, outputTokens: Self.requestedCeiling + Self.outputBelowCeiling,
            lastCallOutputTokens: Self.requestedCeiling, responseTokenCeiling: Self.requestedCeiling)
    ...
    func lastCallCountAboveAttemptCountDoesNotDecide() {
        let reason = FinishReason(
            submissionEntries: [], outputTokens: 0, lastCallOutputTokens: Self.requestedCeiling,
            responseTokenCeiling: Self.requestedCeiling)

        #expect(reason == .completed)
    }

    // MARK: - The whole answer over a live session backend

    @Test("an answer whose backend reports incompleteOutput below the ceiling closes with finishReason endedInsideReasoning")
    func answerEndedInsideReasoningReportsEndedInsideReasoning() async throws {
    ...
    @Test("an answer whose backend reports incompleteOutput at the ceiling closes with finishReason maxTokens")
    ...
    @Test("an answer whose model finishes closes with finishReason completed")
    func finishedAnswerReportsCompleted() async throws {
        let fixture = try await CeilingProbeSessionFixture.make(
    ...
    @Test(
        "an answer that reaches the ceiling in its answer text with no metadata closes with finishReason maxTokens",
        arguments: [nil, SubmissionFinishReasonTests.requestedCeiling])
    ...
    @Test("a streamed answer that reaches the ceiling in its answer text closes with finishReason maxTokens")
    func streamedAnswerTruncatedAtCeilingReportsMaxTokens() async throws {
        let fixture = try await CeilingProbeSessionFixture.make(
            ending: .truncatedInAnswerText, tempDirPrefix: Self.tempDirPrefix)
        defer { try? FileManager.default.removeItem(at: fixture.directory) }

        let usage = try #require(await Self.closingUsage(ofStreamedAnswerOn: fixture.session))
    ...
    ///
    /// - Parameter session: The session to drive the answer on.
    ...
    private static func closingUsage(ofStreamedAnswerOn session: RoutedSession) async throws -> TokenUsage? {
    ...
    @Test(
        "a tool-calling answer whose last call reaches the ceiling in its answer text closes with finishReason maxTokens",
        arguments: [nil, SubmissionFinishReasonTests.requestedCeiling])
    ...
    @Test("a tool-calling answer whose earlier call spends the ceiling and whose last call finishes closes with completed")
    ...
    @Test("a streamed tool-calling answer whose last call reaches the ceiling in its answer text closes with maxTokens")
    ...
    @Test("a streamed tool-calling answer whose last call sends no text does not read the count of the earlier call")
