...
struct TurnOutcomeReducer {
    ...
    mutating func apply(_ event: SessionEvent) {
        ...
        case .turnStarted, .reasoningDelta, .entryRecorded, .discoveryPrimingFailed,
            .generationStalled, .submissionQueued, .submissionStarted, .repetitionStopped, .runSettled,
            .toolCallReport, .elicitationRequested, .generationCall:
            // Deliberately not carried by the outcome: the frame, the priming
            // report, the stall report, the report of the wait of a submission
            // for the worker and of its start, the repetition stop report (the
            ...
            // question a host answers while the turn runs. Reasoning is model
