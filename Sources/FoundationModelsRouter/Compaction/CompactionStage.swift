...
/// ``Compactor/compact(_:budget:)`` try a stage, measure the result, and stop
/// as soon as the transcript is small enough, without ever worrying about
/// having mutated something it shouldn't.
///
/// Only the deterministic stages (``ToolOutputElision``, ``TurnTruncation``)
/// conform here. The model-assisted `Summarization` stage (a later
/// build-order step) is async and needs a prompt and a summarizer model, so
/// it wires into the pipeline through a different mechanism rather than this
