...
/// and only this one fact is promoted to the envelope.
///
/// The authority distinction between `.stopped`, `.cancelled` and `.lost` is
/// deliberate and must not be flattened: `.stopped` is authoritative
/// termination (Shelltool's `killpg(SIGKILL)` on the child's own process
/// group), `.cancelled` is only a request (MCP's advisory
/// `notifications/cancelled`, honestly "we stopped listening"), and `.lost` is
/// unknowable (MCP's transport drop, where a `ProgressToken` is meaningless
/// across connections).
///
/// Not every emitting tool can produce every case. What matters is that each
/// case means the same one thing to every consumer, and that downstream
/// mappings respect the distinction: the ACP agent maps `.lost` to its own
/// `_lost` status, never to `.failed`, because "we do not know if this ran" is
/// not the claim "this ran and failed".
///
/// Unknown-preserving: an unrecognized wire value decodes to `.other(_)`
/// instead of failing, mirroring ACP's `_`-prefixed extensible-enum position,
/// so a tool can ship a novel outcome without a lockstep release of this
/// package.
public enum OperationOutcome: Sendable, Equatable {
    ...
    /// The wire-format vocabulary for every case except `.other`, consulted
    /// from both `rawValue` and `init(rawValue:)` so the case-to-string
...
extension OperationOutcome: Codable {
    /// Decodes an outcome from a bare wire-format string.
