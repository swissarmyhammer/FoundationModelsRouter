...
**Answer:** Router commits cbe41e1 and 6ddc873 make `SessionEvent.runSettled` fire for a nested run under the run's own `completionToken`. The mailbox forwards each settled run's terminal event through the Router-internal `BackgroundRunSettlementObserver` in `Sources/FoundationModelsRouter/Session/OperationEventJournal.swift`. `SessionMailbox.markSettled` hands the settled terminal to the observer, and the session writes it with `SessionOutbox.journalWithoutStaging(event:)` in `Sources/FoundationModelsRouter/Session/RoutedSessionActorRunJournal.swift`. The journal refuses a second terminal for the same correlation, so the write is safe. The suites `NestedRunTerminalForwardingTests`, `SessionMailboxTests`, and `MountedRunSweptTerminalTests` in `Tests/FoundationModelsRouterTests` show this. Known limit: a run that the `close()` sweep already removed drops its late natural terminal, and the sweep's built terminal is the one the journal holds.
